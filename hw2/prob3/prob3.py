...
if __name__ == '__main__':
    ...
    fig = plt.figure()
    plt.title("Eigenvalue Decomposition",fontsize=fs,fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('prob3_ev_N_compare.pdf', format='pdf', dpi=1000, bbox_inches="tight", pad_inches=0)
    plt.clf()
    ...
    # Plot the original and averaged noise matrices
    fig = plt.figure()
    plt.title("Cholesky Decomposition",fontsize=fs,fontweight='bold')
    ax1 = plt.subplot(1, 2, 1)
    ax1.imshow(N)
    ax1.set_title("Original N")
    ax2 = plt.subplot(1, 2, 2)
    ...
    plt.tight_layout()
    #plt.subplots_adjust(top=0.95)
    plt.savefig('prob3_chol_N_compare.pdf', format='pdf', dpi=1000, bbox_inches="tight", pad_inches=0)
